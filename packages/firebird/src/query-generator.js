...
import { FirebirdQueryGeneratorTypeScript } from './query-generator-firebird.internal.js';  // Change to Firebird generator

const typeWithoutDefault = new Set(['BLOB', 'TEXT', 'GEOMETRY', 'JSON']);  // Firebird handles these types similarly

export class FirebirdQueryGenerator extends FirebirdQueryGeneratorTypeScript {
  createTableQuery(tableName, attributes, options) {
    options = {
      // Default options are typically specific to the DB engine
      ...options,
    };
    ...
    for (const attr in attributes) {
      ...
      if (dataType.includes('PRIMARY KEY')) {
        ...
        if (dataType.includes('REFERENCES')) {
          // Firebird doesn't support inline REFERENCES declarations: move to the end
    ...
    return joinSQLFragments([
      ...
      table,
      `(${attributesClause})`,
      options.charset && `CHARSET ${options.charset}`,  // Firebird uses a simpler charset definition
  ...
  attributeToSQL(attribute, options) {
    ...
    // Handle types in Firebird that can't have default values
    if (
